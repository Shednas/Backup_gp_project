    ...
    <div id="add-user-tab" class="tab-content <?= $active_tab == 'add-user' ? 'active' : '' ?>">
        <div class="form-container">
            ...
            <form method="POST" id="addUserForm">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <div style="display: flex; align-items: center; gap: 8px;">
                                <input type="text" id="addUserPassword" name="password" readonly placeholder="Please Generate the password">
                                <button type="button" id="generatePasswordBtn" class="btn btn-secondary" style="white-space: nowrap;">Generate</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- Course Assignment -->
                <div class="form-section course-selection-section hidden" id="courseSelection" style="display: none;">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        if ($user['id'] == $current_user['id']) {
                                            $can_delete = false;
                                            $delete_reason = "Current User";
                                        }
                                        // Cannot delete super admin unless current user is also super admin
                                        elseif ($user['is_super_admin'] && !$current_user_is_super_admin) {
                                            $can_delete = false;
                                            $delete_reason = "Protected Account";
                                        }
                                        // Cannot delete user ID 1 (primary admin)
                                        elseif ($user['id'] == 1) {
                                            $can_delete = false;
                                            $delete_reason = "Primary Admin";
                                        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (generateBtn) {
        generateBtn.addEventListener('click', function() {
            ...
            passwordInput.type = 'password'; // Switch to password type after generation
        });
        // Show placeholder if empty
        passwordInput.addEventListener('input', function() {
            if (!passwordInput.value) {
                passwordInput.placeholder = 'Please generate the password';
                passwordInput.type = 'text'; // Show as text when empty
            }
        });
    }
    ...
    function updateCourseSelection() {
        ...
        if (selectedUserType === 'student' || selectedUserType === 'lecturer') {
            courseSelection.style.display = 'block';
            courseLabel.textContent = selectedUserType === 'lecturer' ? 'Assign Courses to Teach' : 'Enroll in Courses';
        } else {
    ...
    userTypeRadios.forEach(radio => {
        radio.addEventListener('change', updateCourseSelection);
    });
});
</script>