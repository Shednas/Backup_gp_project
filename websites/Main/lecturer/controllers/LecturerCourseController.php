...
if ($_POST) {
    ...
    if (isset($_POST['delete_material'])) {
        ...
        if ($material_id) {
            try {
                ...
                $material = fetchOne("SELECT file_path FROM course_materials WHERE id = ? AND course_id = ?", [$material_id, $course_id]);
                if ($material) {
                    // Delete file from filesystem
                    $file_path = __DIR__ . '/../../public' . $material['file_path'];
                    if (file_exists($file_path)) {
                        unlink($file_path);
                    }
                    // Delete from database
                    query("DELETE FROM course_materials WHERE id = ? AND course_id = ?", [$material_id, $course_id]);
    ...
    if (isset($_POST['delete_announcement'])) {
        ...
        if ($announcement_id) {
            try {
                query("DELETE FROM course_announcements WHERE id = ? AND course_id = ?", [$announcement_id, $course_id]);
    ...
    if (isset($_POST['delete_assignment'])) {
        ...
        if ($assignment_id) {
            try {
                // Get assignment info to delete file
                $assignment = fetchOne("SELECT file_path FROM assignments WHERE id = ? AND course_id = ?", [$assignment_id, $course_id]);
                if ($assignment) {
                    // Delete file from filesystem if exists
                    if ($assignment['file_path']) {
                        $file_path = __DIR__ . '/../../public' . $assignment['file_path'];
                        if (file_exists($file_path)) {
                            unlink($file_path);
                        }
                    }
                    // Delete related submissions and grades first
                    query("DELETE FROM course_grades WHERE assignment_id = ?", [$assignment_id]);
                    
                    // Get submission files to delete
                    $submissions = fetchAll("SELECT file_path FROM assignment_submissions WHERE assignment_id = ?", [$assignment_id]);
                    foreach ($submissions as $submission) {
                        if ($submission['file_path']) {
                            $sub_file_path = __DIR__ . '/../../public' . $submission['file_path'];
                    ...
                    query("DELETE FROM assignment_submissions WHERE assignment_id = ?", [$assignment_id]);
                    query("DELETE FROM assignments WHERE id = ? AND course_id = ?", [$assignment_id, $course_id]);
