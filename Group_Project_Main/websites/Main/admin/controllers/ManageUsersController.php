        ...
        if (isset($_POST['preview_user_type'])) {
            ...
            if ($username && $password && $email && $full_name && $user_type) {
                ...
                if ($existing_user) {
                    ...
                    $check_admin = $user_type === 'admin' ? 1 : 0;
                    $check_student = $user_type === 'student' ? 1 : 0;
                    $check_lecturer = $user_type === 'lecturer' ? 1 : 0;

                    query("INSERT INTO users (username, password, email, full_name, check_admin, check_student, check_lecturer) VALUES (?, ?, ?, ?, ?, ?, ?)",
                          [$username, $hashed_password, $email, $full_name, $check_admin, $check_student, $check_lecturer]);
...
$lecturers = fetchAll("SELECT * FROM users WHERE check_lecturer = 1 ORDER BY full_name ASC");
try {
