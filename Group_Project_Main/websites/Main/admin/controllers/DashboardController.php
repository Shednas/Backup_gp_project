...
// Assuming session and db included already
...
$total_students = fetchOne("SELECT COUNT(*) as count FROM users WHERE check_student = 1")['count'] ?? 0;
$total_lecturers = fetchOne("SELECT COUNT(*) as count FROM users WHERE check_lecturer = 1")['count'] ?? 0;
